...
import { createReceptionistReportBlob, downloadPdf, openPdf } from "../../../../../services/utils/helper-functions/pdf"
...
import { useCreateDataToPostReceptionists } from "../../../../../services/hooks/use-create-data-to-post-receptionists"
...
const CreateReportButton: FC<ICreateReportButton> = (props) => {
    const data = useCreateDataToPostReceptionists(props.data)
    const [upload] = useUploadDocumentReportMutation()
    const [documentId, setDocumentId] = useState<string | undefined>(undefined)
    const [getDocument, {isFetching}] = useLazyGetDocumentQuery()

    const handleClick = async () => {
        const blob = await createReceptionistReportBlob(data, props.isAdmin)
        try {
